...
class GiftRecommendationTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "予算に応じて麻布台ヒルズ内で購入可能なギフトを提案します。"
            "価格帯、受取人の属性、シーンに応じた商品やサービスを推奨します。"
        )
        ...
        self.gift_categories = {
            "グルメ・スイーツ": {
                ...
                "中予算": (
                    3000,
                    8000,
                    ["高級洋菓子セット", "ワインやシャンパン", "グルメ弁当やオードブル", "プレミアムコーヒーセット"],
                ),
                "高予算": (
                    8000,
                    20000,
            ...
            "ファッション・アクセサリー": {
                "低予算": (
                    2000,
                    5000,
                    ["ハンドタオルやハンカチ", "アクセサリー小物", "コスメティック用品", "スカーフやストール"],
                ),
                "中予算": (
                    5000,
                    12000,
                ...
                "高予算": (
                    12000,
                    30000,
                    ["高級ブランド小物", "プレミアムジュエリー", "デザイナーアクセサリー", "特別オーダー商品"],
                ),
            },
            "体験・サービス": {
                "低予算": (
                    3000,
                    6000,
                    ["カフェやレストランの食事券", "書籍や雑貨", "スパやエステの体験券（短時間）", "映画鑑賞券"],
                ),
                "中予算": (
                    6000,
                    15000,
        ...
        try:
            ...
            if not budget:
                return {
                    "success": False,
                    "message": (
                        "予算が明確でありません。「〇〇円で」「予算〇〇円」のように" "具体的な金額を教えてください。"
                    ),
                    "suggestions": [
