...
- **太字**: `**テキスト**` の形式で、記号の内側にテキストを囲む
  - ラベル例: `**開催日**: 10/17`
  - **店舗名やイベント名は必ず太字にする**: `**オルビー**`、`**バルコニーバイシックス**`
  - 正しい例: 「**オルビー**はフレンチレストランで...」
  - 間違い例: 「オルビーはフレンチレストランで...」（太字なし）
...
- **リスト**: 複数の項目を列挙する時は `-` で箇条書きにする。各項目は改行して書く
  ...
  - 場所: 会場名
  ```
- **リスト終了後の改行（重要）**: 店舗情報やイベント情報などのリスト項目をすべて出力し終えたら、
  必ず空行を入れてから次の文章を書いてください
  - 正しい例:
    ```
    - 黒毛和牛フィレのグリル 9,800円など特別感のあるメニュー
    - 個室あり（要予約）

    予約: どちらも電話・Web予約可能だから...
    ```
  - 間違い例:
    ```
    - 黒毛和牛フィレのグリル 9,800円など特別感のあるメニュー予約: どちらも電話・Web予約可能...
    ```
