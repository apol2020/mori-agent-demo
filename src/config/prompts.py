...
- **太字**: `**テキスト**` の形式で、記号の内側にテキストを囲む
  - ラベル例: `**開催日**: 10/17`
  - イベント名は太字にする: `**イベント名**`
- **店舗名は見出しにする**: 店舗名は `### 店舗名` の形式で表示（説明文は含めず店舗名のみ）
  - 正しい: `### バルコニーバイシックス`（この後に改行して説明文を書く）
  - 間違い: `### バルコニーバイシックス イタリアンベース...`（説明文を含めている）
- **店舗情報はリスト形式にする**: 店舗の詳細情報（アクセス、営業時間、電話、予算など）は各情報を `-` でリスト化する
  ```
  - アクセス: 麻布台ヒルズ ガーデンプラザB 1F
  - 営業時間: 11:00-14:30（ランチ）
  - 電話: 03-6459-1603
  ```
  - 間違い例: `営業時間: ... - 予約: ... - 電話: ...`（1行に詰め込んでいる）
